...
total_detections_min = 5*dir_len
total_detections_max = 10*dir_len
...
sev_threshold_low = 0.0
sev_threshold_high = 0.5

while True:
	# sev_threshold == sev_threshold/2
	sum_detect = 0
	sum_cluster = []
	...
	for i in range(dir_len-iter_):
		...
		if i == 0:
			...
			pos_neg_clstr.append(sum_pos_neg)
			sum_cluster.append(sum_cstr)
	if (sum(elements_per_cluster)>total_detections_max):
		print("total anomalies",sum(elements_per_cluster))
		print("total detections max",total_detections_max)
		sev_threshold = sev_threshold_low + (sev_threshold_high - sev_threshold_low )/2
		sev_threshold_high = sev_threshold_low + (sev_threshold_high - sev_threshold_low )/2
			
	elif (sum(elements_per_cluster)<total_detections_min):
		print("no")
		sev_threshold_low = sev_threshold_low + (sev_threshold_high - sev_threshold_low )/2
		sev_threshold = sev_threshold_low + (sev_threshold_high - sev_threshold_low )/2
	if (sum(elements_per_cluster)<total_detections_max and sum(elements_per_cluster)>total_detections_min):
		break
